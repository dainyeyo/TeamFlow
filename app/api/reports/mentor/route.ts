  ...
  try {
    ...
    if (apiKey) {
      try {
        ...
        const prompt = `
당신은 IT 실무 부서의 전문 애자일 코치(Agile Coach)이자 프로젝트 리스크 매니지먼트 전문가입니다.
...
5. suggestions (PM과 팀을 위한 애자일 코칭 가이드): 이 팀을 코칭할 때 어떤 방향으로 피드백해야 기술-비기술 직군 간 소통이 원활해지고 생산성 높은 마감이 가능할지 조언 사항을 3~4가지 제안하십시오.
