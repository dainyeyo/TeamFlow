...
export async function GET(req: Request) {
  try {
    ...
    let suggestions: string[] = [
      ...
      "멘토 코칭 피드백 타임라인을 참고하여 주기적으로 지도를 진행해 주세요."
    ];
    ...
    if (apiKey) {
      try {
        ...
        const prompt = `
당신은 대학생 AI 부트캠프 프로젝트의 전문 기술 멘토이자 팀 빌딩 컨설턴트입니다.
...
- 역할군별 태스크 개수:
  - 기획 (비전공 영역): ${planning}개
  - 데이터 (비전공/전공 교차): ${data}개
  - AI 모델링 (전공 영역): ${modelTasks}개
  - 웹 개발 (전공 영역): ${devTasks}개

[진단 기준]
1. 개발/모델링 태스크에 비해 기획/데이터 수집 태스크가 지나치게 적거나 많다면 협업 불균형 리스크로 판단합니다.
...
5. suggestions (멘토를 위한 코칭 가이드): 이 팀을 지도할 때 어떤 방향으로 피드백해야 전공-비전공자 소통이 원활해지고 마감이 가능할지 조언 사항을 3~4가지 제안하십시오.
